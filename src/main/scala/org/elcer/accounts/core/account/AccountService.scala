...
class AccountService(accountStorage: AccountStorage
  ...
  def createAccount(account: Account): Future[Account] = accountStorage.saveAccount(account)

  def transferFunds(from: Account, to: Account, amount: BigDecimal): Future[Unit] =
    accountStorage.transferFunds(from, to, amount)

  def updateAccount(id: Long, accountUpdate: AccountUpdate): Future[Option[Account]] =
    accountStorage
      .getAccount(id)
